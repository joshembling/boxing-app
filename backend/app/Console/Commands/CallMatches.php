...
class CallMatches extends Command
{
    ...
    public function handle()
    {
        ...
        $data = $crawler->filter('.fight-card')->each(function ($fightCard) {
            ...
            // HEADSHOTS
            $headshots = $fightCard->filter('.fight-card__fighters .fight-card__photo')->each(function ($headshot, $key) use ($names) {
                if ($headshot->filter('img')->count() > 0) {
                    // return $headshot->filter('img')->attr('data-src');

                    $url = $headshot->filter('img')->attr('data-src');
                    $contents = file_get_contents($url);
                    $file_extension = substr($url, strrpos($url, '.') + 1);
                    $name = str_replace(' ', '_', $names[$key]) . '.' . $file_extension;
                    Storage::get('/public/headshots/' . $name) ?? Storage::put('/public/headshots/' . $name, $contents);
                }
            });
            ...
            $flags = $fightCard->filter('.fight-card__flag')->each(function ($flag) {
                if ($flag->filter('img')->count() > 0) {
                    $url = $flag->filter('img')->attr('data-src');
                    $contents = file_get_contents($url);
                    $name = substr($url, strrpos($url, '/') + 1);
                    Storage::get('/public/flags/' . $name) ?? Storage::put('/public/flags/' . $name, $contents);
        ...
        foreach ($data as $d) {
            ...
            $b1_name = $d['names'][0];
            $b1_name_formatted = str_replace(' ', '_', $b1_name);
            $b1_flag = $d['flags'][0]['img'];
            $b1_flag_formatted = substr($b1_flag, strrpos($b1_flag, '/') + 1);

            $b2_name = $d['names'][1];
            $b2_name_formatted = str_replace(' ', '_', $b2_name);
            $b2_flag = $d['flags'][1]['img'];
            $b2_flag_formatted = substr($b2_flag, strrpos($b2_flag, '/') + 1);
            ...
            $boxer_data1 = [0 => [
                ...
                'headshot' => str_contains(Storage::url('public/headshots/' . $b1_name_formatted . '.png'), $b1_name_formatted) ? Storage::url('public/headshots/' . $b1_name_formatted . '.png') : null,

                'flag' => [
                    str_contains(Storage::url('public/flags/' . $b1_flag_formatted), $b1_flag_formatted) ? Storage::url('public/flags/' . $b1_flag_formatted) : null,
                    $d['flags'][0]['alt'],
                ],
            ...
            $boxer_data2 = [1 => [
                ...
                'headshot' => str_contains(Storage::url('public/headshots/' . $b2_name_formatted . '.png'), $b2_name_formatted) ? Storage::url('public/headshots/' . $b2_name_formatted . '.png') : null,

                'flag' => [
                    str_contains(Storage::url('public/flags/' . $b2_flag_formatted), $b2_flag_formatted) ? Storage::url('public/flags/' . $b2_flag_formatted) : null,
                    $d['flags'][1]['alt'],
                ],
        ...
        foreach ($matches as $m) {
            ...
            if ($BoxingMatch) {
                ...
                $BoxingMatch->boxer_1_flag_img = $m['boxers'][0]['flag'][0] ?? null;
                $BoxingMatch->boxer_1_flag_alt = $m['boxers'][0]['flag'][1] ?? null;
                ...
                $BoxingMatch->boxer_2_flag_img = $m['boxers'][1]['flag'][0] ?? null;
                $BoxingMatch->boxer_2_flag_alt = $m['boxers'][1]['flag'][1] ?? null;
